...
func GeneratePlotFor(pricePairs []csvprocess.EthereumPrice) (*plot.Plot, error) {
	...
	err := plotutil.AddLinePoints(pricePlot, "ETH", points)

	if err != nil {
		return nil, fmt.Errorf("error adding line in plot: %w", err)
	}
